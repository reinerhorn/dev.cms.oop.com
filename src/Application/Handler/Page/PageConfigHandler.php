...
final class PageConfigHandler implements CrudHandlerInterface
{
    ...
    public function save(array $data): string
    {
        ...
        $pageSlug = $this->resolvePageSlug($pageUuid, $data['fk_page_slug'] ?? null);
    ...
    /**
     * fk_page_slug is derived from the selected page and must not be trusted
     * from form input. The optional submitted value is ignored deliberately.
     */
    private function resolvePageSlug(string $pageUuid, mixed $submittedSlug = null): string
    {
        $stmt = $this->db->prepare("
            SELECT slug
            FROM page
        ...
        if (!$stmt) {
            throw new RuntimeException(
                'Page slug lookup prepare fehlgeschlagen: ' . $this->db->error
            );
        }
        ...
        if ($slug === '') {
            throw new RuntimeException(
                'Keine Seite mit page_uuid gefunden: ' . $pageUuid
            );
        }
