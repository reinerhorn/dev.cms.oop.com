...
class Navigation
{
    ...
    public function getItems(): array
    {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            if (!$this->isAuthVisible($row['auth_visibility'] ?? 'public')) {
                continue;
            }
    ...
    private function isAuthVisible(string $visibility): bool
    {
        return match ($visibility) {
            'public'    => true,
            'guest'     => ($this->roleId === '' || $this->roleId === 'guest-role-000'),
            'logged_in' => ($this->roleId !== '' && $this->roleId !== 'guest-role-000'),
            default     => false,
        };
    }
}