...
error_log('🔥 TEST LOG');
error_log('🔥 CMS LOG TEST 1');
error_log('🔥 CMS LOG TEST 2');
...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') {
    ...
    $formResult = $dispatcher->dispatch($_POST, []);'*/

    $_SESSION['form_result'] = $formResult;

    $redirect = $formResult['redirect'] ?? ($_SERVER['HTTP_REFERER'] ?? '/');
