...
echo "<h3>🔄 Starte SEO-Slug-Update (Einzeldurchlauf ohne Trigger)...</h3>";
...
$updated = 0;
$stmtSelect = $mysqli->prepare("SELECT nav_uuid FROM navigation WHERE fk_translation_placeholder = ? AND (seo_slug IS NULL OR seo_slug = '')");
$stmtUpdate = $mysqli->prepare("UPDATE navigation SET seo_slug = ? WHERE nav_uuid = ?");

foreach ($mapping as $placeholder => $slug) {
    $stmtSelect->bind_param('s', $placeholder);
    $stmtSelect->execute();
