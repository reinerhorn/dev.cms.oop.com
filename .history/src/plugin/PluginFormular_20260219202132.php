    ...
    public static function loadByUuid(mysqli $db, string $pluginContentUuid, string $language): array
    {
        ...
        if (!empty($raw['fields']) && is_array($raw['fields'])) {
            foreach ($raw['fields'] as $fieldName => $definition) {
                ...
                // Fallback für alte String-Struktur
                ...
                // Neue Objekt-Struktur (key => definition)
                if (!isset($definition['name'])) {
                    ...
                    $field = [
                        ...
                        'label'    => is_array($label)
                            ? ($label['text'] ?? ucfirst($fieldName))
                            : (string)$label,
                        'required' => !empty($ui['required']),
                        'options'  => []
                    ];
                }
                // Alte numerische Struktur: [{ name, type, required }]
                else {

                    $field = [
                        'name'     => $definition['name'],
                        'type'     => $definition['type'] ?? 'text',
                        'label'    => ucfirst($definition['name']),
                ...
                if (
                    ...
                    // einfache Absicherung gegen leere Werte
                    if ($table && $valueField && $labelField) {

                        // Grundlegende Whitelist-Validierung (nur a-z0-9_ erlaubt)
                        if (
