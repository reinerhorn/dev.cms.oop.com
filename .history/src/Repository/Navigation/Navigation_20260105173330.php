...
class Navigation
{
    ...
    public function __construct(
        ...
        // Admin sieht IMMER Admin-Navigation – unabhängig vom Seitenkontext
        if ($roleLevel >= 2) {
            $this->context = 'admin';
        } else {
     ...
     */
    private function getChildren(?string $parentUuid): array
    {
        $sql = "
            SELECT
                n.nav_uuid,
            ...
            WHERE n.enabled = 1
              AND n.context = ?
              AND COALESCE(n.visible_from_role, 0) <= ?
              AND n.parent_id <=> ?
        ...
        foreach ($rows as $row) {
            $children = $this->getChildren($row['nav_uuid']);
