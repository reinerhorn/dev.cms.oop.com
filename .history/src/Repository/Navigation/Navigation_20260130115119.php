...
class Navigation
{
    ...
    public function getItems(): array
    {
        ...
        $stmt->bind_param('sss', $this->language, $this->context, $this->context);
        $stmt->execute();
    ...
    private function getChildren(string $parentId): array
    {
        ...
        $stmt->bind_param('ssss', $this->language, $this->context, $this->context, $parentId);
    ...
    private function isAllowed(?string $requiredPermissionId): bool
    {
        if ($requiredPermissionId === 'perm-can-logout') {
            return $this->roleId !== '' && $this->roleId !== 'guest-role-000'
                && $this->roleService->roleHasPermission($this->roleId, 'perm-can-logout');
        }
        ...
        // Explizit für Gäste freigegeben → immer sichtbar
        if ($requiredPermissionId === 'perm-view-guest') {
            return true;
        }

        // Keine Role-ID vorhanden → alles andere blockieren
        if ($this->roleId === '' || $this->roleId === null) {
            return false;
        }

        // Normale Permission-Prüfung
