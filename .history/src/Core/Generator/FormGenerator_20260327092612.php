...
class FormGenerator
{
    ...
    private function detectRelationLabel(string $table): string
    {
        $result = $this->db->query("SHOW FULL COLUMNS FROM `$table`");

        $priority = ['name', 'title', 'label', 'username', 'email'];
    ...
    public function generateFormConfig(string $table): array
    {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            if ($fk !== null) {
                ...
                $field['relation_label'] = $this->detectRelationLabel($fk['table']);
            }

            if (str_starts_with($fieldName, 'fk_') && !isset($field['relation'])) {
                $relationTable = str_replace('fk_', '', $fieldName);

                $field['type'] = 'select';
                $field['relation'] = $relationTable;
                $field['relation_key'] = 'id';
                $field['relation_label'] = $this->detectRelationLabel($relationTable);
            }
