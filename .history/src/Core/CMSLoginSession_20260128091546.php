...
final class CMSLoginSession
{
   ...
   public static function handleUserAction(array $data): array
    ...
    public static function login(array $data): array
    {
        ...
        error_log('LOGIN DEBUG: user_row=' . var_export($user, true));
        ...
        error_log('LOGIN DEBUG: session_user_id=' . ($_SESSION['user_id'] ?? 'NULL'));
        error_log('LOGIN DEBUG: session_role_id=' . ($_SESSION['role_id'] ?? 'NULL'));

        $redirect = $_SESSION['login_redirect'] ?? null;
        unset($_SESSION['login_redirect']);

        // Redirect auf Login-Seite ignorieren
        // (Member/Admin sollen auf ihre Default-Seite)
        if (is_string($redirect) && preg_match('#/login$#', $redirect)) {
            $redirect = null;
        }
        ...
        error_log('LOGIN DEBUG: final_redirect=' . ($redirect ?? '/de/startseite'));
