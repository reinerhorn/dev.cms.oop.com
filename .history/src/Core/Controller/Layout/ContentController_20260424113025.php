...
class ContentController
{
  ...
  ");
        ...
        foreach ($rows as $row) {
            ...
            // ------------------------------
            // FORM BUTTON RESOLVER (Service Layer)
            // ------------------------------
            if (!empty($block) && strtolower($pluginName) === 'forms') {

                error_log("🔥 FORMS PLUGIN HIT");
                error_log("PLUGIN KEY: " . $pluginName);

                // form_id must come from config (plugin payload)
                ...
                if ($formId) {
                    ...
                    error_log("➡️ CALL ButtonService for: " . $formId);
                    ...
                    $block['config']['buttons'] = $buttons;

                } else {
                    error_log("❌ NO form_id FOUND in block[config]");
                }
            }
