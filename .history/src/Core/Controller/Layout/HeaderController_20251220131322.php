...
class HeaderController
{
    ...
    public static function getHeaderData(?int $role = 0): array
    {
        ...
        // 1️⃣ Exakter Header für Rolle
        $stmt = $db->prepare("
            SELECT id, headline, fk_translation_placeholder, link, css, role
            FROM header
            WHERE role = ?
            LIMIT 1
        ");
        ...
        return [
            'id'          => $header['id'],
            'headline'    => $header['headline'], // optional statisch
            'placeholder' => $header['fk_translation_placeholder'] ?? null,
            'link'        => $header['link'],
