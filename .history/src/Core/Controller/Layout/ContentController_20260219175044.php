    ...
    private function loadPluginsForPage(string $pageId): array
    {
        ...
        foreach ($rows as $row) {
            ...
            if ($table === 'p_content_formular') {
                ...
                // Neues Plugin-Pattern verwenden
                if (class_exists('\\CMS\\Plugin\\PluginFormular')) {

                    $block = \CMS\Plugin\PluginFormular::loadContentByLanguage(
                        $this->db,
                        $uuid,
                        $this->language
                    );
                    ...
                    if (!empty($block)) {
                        ...
                        // Plugin-Typ wieder korrekt auf "forms" setzen,
                        // damit Twig pages/forms.twig rendert
                        $block['type'] = $plugin;
