...
class ContentController
{
    ...
    private function loadPluginsForPage(string $pageId): array
    {
        ...
        foreach ($rows as $row) {
           ...
           // 🔥 SPEZIAL: Forms → Form-ID setzen + Buttons la den
            if ($plugin === 'forms') {

                // 🔥 form_id direkt aus DB holen (KEIN UUID-Fallback!)
                $formId = null;
                ...
                if ($stmtForm) {
                    $stmtForm->bind_param('s', $uuid);
                    $stmtForm->execute();
                    $resultForm = $stmtForm->get_result()->fetch_assoc();
                    $stmtForm->close();

                    if (!empty($resultForm['config'])) {
                        $configJson = json_decode($resultForm['config'], true);

                        if (is_array($configJson) && !empty($configJson['form_id'])) {
                            $formId = $configJson['form_id'];
                        }
                    }
                }

                error_log('FORM ID FROM DB (REAL): ' . $formId);

                $block['config']['form_id'] = $formId;

                error_log('FINAL FORM ID USED: ' . $formId);
