...
class ContentController
{
    ...
    private function loadPluginsForPage(string $pageId): array
    {
        ...
        foreach ($rows as $row) {
            ...
            // --------------------------------------------
            // Handler-Class direkt verwenden (Override)
            // --------------------------------------------
            if (!empty($handlerClass)) {

                $class = '\\CMS\\Controller\\Admin\\' . $handlerClass;

                if (class_exists($class) && method_exists($class, 'loadContentByLanguage')) {

                    error_log('HANDLER CLASS USED (OVERRIDE): ' . $class);
            ...
            if (!empty($handlerClass)) {
               ...
               $tryController = '\\CMS\\Controller\\Admin\\' . $handlerClass;
