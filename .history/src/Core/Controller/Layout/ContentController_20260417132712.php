...
class ContentController
{
    ...
    private function loadPluginsForPage(string $pageId): array
    {
        ...
        error_log('PLUGIN ROWS COUNT: ' . count($rows));
        $blocks = [];
        ...
        foreach ($rows as $row) {
            ...
            $handlerClass = $this->buildHandlerClass($data['type'], $data['module'], $data['handlerRaw']);

            $block = $this->executeHandler($handlerClass, $data['uuid']);
            if ($block === null) {
                error_log("⚠️ CC EMPTY BLOCK: {$handlerClass}");
                continue;
            }
    ...
    private function mapRow(array $row): array
    {
        return [
            ...
            'type'       => trim((string)($row['type'] ?? 'FormData'))
        ];
    }
    ...
    private function buildHandlerClass(string $type, string $module, string $handler): string
    {
        $base = 'CMS\\Application\\';

        // 🔥 harte Bereinigung gegen \n, \r, Tabs, Leerzeichen
        $type    = trim(str_replace(["\r", "\n", "\t"], '', $type));
        $module  = trim(str_replace(["\r", "\n", "\t"], '', $module));
        $handler = trim(str_replace(["\r", "\n", "\t"], '', $handler));
