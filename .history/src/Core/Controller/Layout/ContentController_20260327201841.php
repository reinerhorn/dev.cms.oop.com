...
class ContentController
{
    ...
    private function loadPluginsForPage(string $pageId): array
    {
        ...
        foreach ($rows as $row) {
            ...
            // Dynamisch Helfermethode ermitteln
            $methodName = null;
            if ($table === 'p_content_plaintext') {
                $methodName = 'loadPlaintextBlock';
            } elseif ($table === 'p_content_formular') {
                $methodName = 'loadFormularBlock';
            }

            if ($methodName !== null && method_exists($this, $methodName)) {
                $block = $this->$methodName($uuid, $idx, $pluginName);
                if ($block !== null) {
                    $blocks[] = $block;
                }
                continue;
