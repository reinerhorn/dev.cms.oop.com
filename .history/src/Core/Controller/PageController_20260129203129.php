...
class PageController
{
    ...
    {
       error_log('PAGE GUARD CHECK');
error_log('SESSION = ' . json_encode($_SESSION));
error_log('PAGE PERMISSION = ' . ($page['required_permission'] ?? 'NONE'));
error_log('ROLE = ' . ($_SESSION['role_id'] ?? 'NO ROLE')); 
        $this->twig = $twig;
        $this->db = $db;
    ...
    public function handle(string $language, string $slug): void
    {
        ...
        $stmt = $this->db->prepare(
            "SELECT page_uuid, context, nav_id FROM page WHERE slug = ? AND enabled = 1 LIMIT 1"
        );
