...
class LayoutController
{
    ...
    public function getContent(?string $pageId): array
    {
        ...
        if ($pageId && !preg_match('/^[0-9a-fA-F\-]{36}$/', $pageId)) {
        ...
        // page_config laden
        $stmt = $this->db->prepare("SELECT pc.page_config_uuid, pc.idx, pc.fk_plugin_uuid, pc.plugin_content_uuid, p.name AS plugin_name FROM page_config pc LEFT JOIN plugin p ON pc.fk_plugin_uuid = p.plugin_uuid LEFT JOIN page pg ON pg.page_uuid = pc.fk_page_uuid WHERE BINARY pc.fk_page_uuid = BINARY ? OR BINARY pg.slug = BINARY ? ORDER BY pc.idx ASC");
