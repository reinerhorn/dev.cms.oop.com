...
class GeneratorController
{
    ...
    public function create(): void
    {
        ...
        error_log('RAW TABLE POST: ' . print_r($_POST['table'] ?? null, true));
        ...
        if (!$table) {
            ...
            echo "Bitte Tabelle auswählen";
            return;
        }

        if (!preg_match('/^[a-zA-Z0-9_]+$/', $table)) {
            error_log('INVALID TABLE FORMAT: ' . $table);
    ...
    public static function loadContentByLanguage(mysqli $db, string $uuid, string $language): array
    {
        ...
        return [
            'type' => 'forms',
            'form_id' => 'generator_create_form',

            'headline' => 'Formular Generator',

            // 🔥 Buttons wie bei forms.twig
            'buttons' => [
                [
                    'button_type' => 'submit',
                    'variant' => 'primary',
