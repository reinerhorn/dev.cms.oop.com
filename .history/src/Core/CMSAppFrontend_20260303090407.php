...
if (
    ...
    && isset($_POST['action'])
    && isset($_POST['form_id'])
    && $_POST['form_id'] === $pageMeta['form_action']
) {
    error_log('DISPATCH FORM_ACTION RAW: >' . ($pageMeta['form_action'] ?? 'NULL') . '<');
    error_log('FORM_ACTION LENGTH: ' . strlen($pageMeta['form_action'] ?? ''));
