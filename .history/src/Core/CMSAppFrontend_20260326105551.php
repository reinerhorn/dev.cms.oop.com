...
// -------------------------------------------------
// 4) Form Action Dispatch (formulargetrieben, korrekt)
// -------------------------------------------------
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (
        ...
        error_log('DISPATCH FORM_ACTION KEY: >' . $pageMeta['form_action'] . '<');
        error_log('POSTED FORM_ID: >' . $_POST['form_id'] . '<');

        $result = PageFormActionDispatcher::dispatch(
            $pageMeta['form_action'],   // Action-Key aus der Page-Meta
            $_POST,
            $pageMeta
        );
...
$pageContext = trim((string)($pageMeta['context'] ?? ''));
        ...
        // -------------------------------------------------
        // 8b) FormData Hydration (GET + POST, isoliert pro Formular)
        // -------------------------------------------------
            ...
            if ($loader !== null) {
                foreach ($contentData as &$block) {
                    ...
                    if (
                        ...
                        // Standard: nur GET verwenden
                        $requestData = $_GET;

                        // POST nur dann verwenden,
                        // wenn dieses Formular wirklich abgeschickt wurde
                        if (
                            $requestMethod === 'POST'
                            && isset($_POST['form_id'])
                            && isset($block['config']['form_id'])
                            && $_POST['form_id'] === $block['config']['form_id']
                        ) {
        ...
        foreach ($contentData as $block) {
            ...
            // -------------------------------------------------
            // 🔥 BUTTONS LADEN (DB-getrieben für forms)
            // -------------------------------------------------
            if ($block['type'] === 'forms') {
                ...
                error_log('FINAL FORM ID USED: ' . print_r($formId, true));
                ...
                if (!empty($formId)) {
                    ...
                    if ($stmt) {
                        ...
                        error_log('BUTTONS LOADED COUNT: ' . count($buttons));
                    } else {
                        error_log('BUTTON QUERY PREPARE FAILED');
                    }
                } else {
                    error_log('NO FORM ID → NO BUTTONS');
                }
            }

            // ALLE ANDEREN PLUGINS → Twig-Templates (z. B. forms)
            // 🔥 ERWEITERT: Template kann direkt aus dem Block kommen

            $tpl = $block['template'] ?? ('pages/' . $block['type'] . '.twig');
        ...
        error_log('PAGE CONTEXT: ' . $pageContext);
        error_log('NAV ID USED: ' . $navId);
        error_log('LAYOUT HEADER EMPTY: ' . (empty($layout['header']) ? 'YES' : 'NO'));
        error_log('LAYOUT NAV EMPTY: ' . (empty($layout['navigation']) ? 'YES' : 'NO'));
        error_log('LAYOUT FOOTER EMPTY: ' . (empty($layout['footer']) ? 'YES' : 'NO'));
