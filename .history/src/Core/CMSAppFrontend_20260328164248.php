...
final class CMSAppFrontend
{
    public static function render(Environment $twig, string $pageId): string
    {
        try {
            ...
            // 🔥 FORM PROCESSING (Auto-Discovery)
            if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                ...
                if ($formType) {
                    ...
                    $handlerClass = self::resolveFormHandler($formType);
                    ...
                    if ($handlerClass) {
                        ...
                        error_log('FORM RESULT: ' . json_encode($result));
                    } else {
                        error_log('❌ Kein Handler für Form-Type: ' . $formType);
                    }
                }
            }
    ...
    private static function resolveFormHandler(string $type): ?string
    {
        // ❌ No reflection scanning anymore
        // 👉 Simple static mapping (can later be moved to Registry)

        $map = [
            // 'login' => \CMS\Form\LoginHandler::class,
            // 'register' => \CMS\Form\RegisterHandler::class,
        ];

        if (!isset($map[$type])) {
            error_log("❌ Kein FormHandler registriert für: " . $type);
            return null;
        }
