    ...
    public static function render(Environment $twig, string $pageId): string
    {
        try {
            ...
            foreach ($contentData as $block) {
                ...
                if (!isset($block['type'])) {
                    error_log('Missing type in content block idx=' . ($block['idx'] ?? 'unknown'));
                    continue;
                }

                $template = match ($block['type']) {
                    'plaintext' => 'blocks/plaintext.twig',
                    'forms'     => 'blocks/forms.twig',
                    'login'     => 'blocks/login.twig',
                    default      => 'blocks/plaintext.twig',
                };
