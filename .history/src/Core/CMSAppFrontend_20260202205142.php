...
final class CMSAppFrontend
{
    ...
    public static function render(
        ...
        // -------------------------------------------------
        // 0) Global Login-POST Handling (zentral!)
        // -------------------------------------------------
        if (
            $_SERVER['REQUEST_METHOD'] === 'POST'
            && ($_POST['intent'] ?? '') === 'login'
        ) {
            $handler = new \CMS\Application\FormAction\Auth\LoginHandler();
            $result  = $handler->handle($_POST);

            if (($result['success'] ?? false) === true) {
                $_SESSION['user_id'] = $result['user_id'] ?? 'demo-user-001';
                $_SESSION['role_id'] = $result['role_id'] ?? 'member-role-001';

                $redirect =
                    $_POST['redirect']
                    ?? $_SESSION['login_redirect']
                    ?? '/' . (CMSApp::getLanguage() ?? 'de');
            ...
            $_SESSION['login_error'] = $result['message'] ?? 'Login fehlgeschlagen';
        }
        ...
        // -------------------------------------------------
        $stmt = $db->prepare(
            "SELECT page_uuid, context, enabled, auth_visibility, required_permission_id, nav_id, page_css_id
             FROM page
             WHERE page_uuid = ?
             LIMIT 1"
        );
        ...
        // -------------------------------------------------
        // 4) Guards – konsistent über ContextRegistry
        // -------------------------------------------------
        // Login- UND Register-Seiten sind IMMER öffentlich (Guard-Bypass)
        $isPublicAuthPage =
            $pageContext === 'frontend'
            && in_array(($pageMeta['page_css_id'] ?? ''), ['login', 'register'], true);

        // ---------------------------------------------
        // 4a) Auth-Visibility Guard (DB-getrieben)
        // ---------------------------------------------
        $authVisibility = $pageMeta['auth_visibility'] ?? 'public';

        // public  -> immer erlaubt
        // guest   -> nur NICHT eingeloggte
        // logged_in -> nur eingeloggte
        ...
        if (!$isPublicAuthPage && $contextConfig['requires_login'] === true) {
            ...
            if (!empty($pageMeta['required_permission_id'])) {
                if (empty($_SESSION['user_id']) || !self::checkPermission($pageMeta['required_permission_id'])) {
        ...
        if ($contextConfig['requires_login'] === true) {
