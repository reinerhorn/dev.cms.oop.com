...
use CMS\Application\FormData\Admin\EntityFormDataLoader;
...
$pageContext = trim((string)($pageMeta['context'] ?? ''));
        ...
        // -------------------------------------------------
        // 8b) FormData Hydration (GET + POST, zentral)
        // -------------------------------------------------
        ...
        if (!empty($pageMeta['form_action'])) {
            ...
            if ($loader !== null) {
                foreach ($contentData as &$block) {
                    ...
                    // Alle Formular-Blöcke hydratisieren (GET + POST kombiniert)
                    if (
                        isset($block['type'])
                        && $block['type'] === 'forms'
                    ) {
                        error_log(print_r($block['config'], true));
