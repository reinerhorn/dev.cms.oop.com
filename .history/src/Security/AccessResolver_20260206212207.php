...
final class AccessResolver
{
    ...
    /**
     * Zentrale Sicherheitsprüfung für FormActions (DB-getrieben)
     *
     * Regeln:
     * - Action muss über UI-Button definiert sein
     * - Seite muss für Rolle sichtbar sein
     * - Optional: Permission-Prüfung über roleService
     */
    public function assertFormActionAllowed(
        ...
        // 1) Page-Zugriff sicherstellen (nutzt bestehende Logik)
        $this->assertPageAccess($pageMeta, $roleId);

        // 2) Optional: FormAction-spezifische Permission aus pageMeta
        // Erwartet z. B.:
        // $pageMeta['form_action_permissions']['auth_login'] = 'perm-can-login';
        $actionPermissions = $pageMeta['form_action_permissions'][$action] ?? null;

        if ($actionPermissions) {
            if (!$this->roleService->roleHasPermission($roleId, $actionPermissions)) {
                throw new RuntimeException('Keine Berechtigung für diese Aktion');
            }
        }

        // 3) Keine weitere Blockade → Action erlaubt
    }
