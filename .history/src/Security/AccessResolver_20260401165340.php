...
final class AccessResolver
{
    ...
    {
        error_log('FORM ACTION CHECK START');
        error_log('ACTION: ' . ($actionMeta['action'] ?? 'NULL'));
        error_log('PERMISSION: ' . ($actionMeta['permission_id'] ?? 'PUBLIC'));
        ...
        if (empty($actionMeta['permission_id'])) {
            error_log('ACCESS GRANTED: PUBLIC ACTION');
            return;
        }

        // Ab hier: Permission existiert → Login erforderlich
        if ($roleId === self::GUEST_ROLE) {
            error_log('ACCESS DENIED: LOGIN REQUIRED');
            throw new ForbiddenException('Login erforderlich');
        }
        ...
        if (!$this->roleService->roleHasPermission(
            ...
            error_log('ACCESS DENIED: MISSING PERMISSION ' . $actionMeta['permission_id']);
            throw new ForbiddenException('Keine Berechtigung für diese Aktion');
        }

        error_log('ACCESS GRANTED: PERMISSION OK');
    }
