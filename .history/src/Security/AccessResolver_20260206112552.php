...
final class AccessResolver
{
    ...
    /**
     * Zentrale Sicherheitsprüfung für FormActions
     *
     * Regeln:
     * - action muss in pageMeta erlaubt sein
     * - auth erforderlich → User darf kein Guest sein
    ...
    public function assertFormActionAllowed(
        ...
        $allowedActions = $pageMeta['allowed_form_actions'] ?? [];
        ...
        // Falls Seite Auth benötigt → Login erzwingen
        if (($pageMeta['auth_visibility'] ?? 'public') !== 'public') {
            if ($roleId === 'guest-role-000') {
                throw new RuntimeException('Login erforderlich für diese Aktion');
            }
        }

        // Optional: Permission auf Action-Ebene
