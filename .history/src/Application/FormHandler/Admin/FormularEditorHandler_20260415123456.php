...
namespace CMS\Application\FormHandler\Admin;

use CMS\Application\Interface\HandlerInterface;
use mysqli;

class FormularEditorHandler implements HandlerInterface
{
    /**
     * Plugin-Name für Auto-Discovery
     */
        ...
        if (!empty($raw['fields']) && is_array($raw['fields'])) {
            foreach ($raw['fields'] as $definition) {
                ...
                $field = [
                    ...
                    'type'        => $ui['type'] ?? ($definition['type'] ?? 'text'),
