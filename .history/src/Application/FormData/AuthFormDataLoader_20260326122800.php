...
final class AuthFormDataLoader implements FormDataLoaderInterface
{
    ...
    {
        return in_array($formAction, ['auth_login', 'auth_register'], true);
    }
    ...
    public function hydrate(array $block, array $request): array
    {
        ...
        // -----------------------------
        // Form ID sauber über form_action bestimmen
        // -----------------------------
        $formAction = $block['config']['form_action'] ?? null;
        $formId = null;

        if ($formAction === 'auth_login') {
            $formId = 'auth_login_form';
        } elseif ($formAction === 'auth_register') {
            $formId = 'auth_register_form';
        } else {
            // Fallback (nur wenn nichts gesetzt ist)
            $formId = $block['config']['form_id'] ?? 'default_form';
        }
