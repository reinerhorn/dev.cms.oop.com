...
$formId = $postData['form_id'] ?? null;

if (!$formId) {
    throw new RuntimeException('FORM_ID fehlt');
}

$sql = "SELECT module, handler_class
        FROM plugin
        WHERE name = ?
        AND is_active = 1
        LIMIT 1";
...
if (!$result) {
    throw new RuntimeException('Kein Handler für form_id gefunden: ' . $formId);
}
