...
final class RegisterAction implements FormActionInterface
{
    public function handle(array $data): array
    {
        ...
        // 6. Verifizierungs-Mail senden (PHPMailer + bestehende MailConfig)
        try {
            $mailConfigFile = $_ENV['CMS_MAIL_CONFIG'] ?? null;
                    error_log('REGISTER MAIL: reached mail block');
            if (!$mailConfigFile || !is_file($mailConfigFile)) {
                throw new \RuntimeException('CMS_MAIL_CONFIG nicht gefunden oder ungültig');
            }
            ...
            $mail->setFrom(
                $mailConfig['from_address'] ?? 'noreply@dev.cms-oop.com',
                $mailConfig['from_name'] ?? 'CMS'
            );
