...
final class LoginAction implements FormActionInterface
{
    public function handle(array $data): array
    {
        ...
        error_log('LOGIN DATA: id=' . var_export($id, true) . ' role=' . var_export($roleId, true));
        ...
        error_log('LOGIN DEBUG FINAL VALUES:');
        error_log('LOGIN DEBUG FINAL ID = ' . var_export($id, true));
        error_log('LOGIN DEBUG FINAL ROLE = ' . var_export($roleId, true));

        if (empty($id) || empty($roleId)) {
            error_log('LOGIN FATAL: EMPTY ID OR ROLE');
            return [
                'success' => false,
                'message' => 'Login fehlgeschlagen (interner Fehler)'
            ];
        }
