...
<!-- Formular-Vorschau -->
<div class="admin_box">
    <h2>Generiertes Formular</h2>
    <?php if (!empty($selected_formular['id'])):
        ob_start(); ?>
        <form method="post" action="">
        <?php foreach ($felder as $feld):
            if ($feld['fk_formular_id'] === $selected_formular['id']):
                $name = strtolower(preg_replace('/[^a-zA-Z0-9_]/', '_', $feld['label']));
        ...
        <?php
        $generatedForm = ob_get_clean();
        echo $generatedForm;

        // Ordner bestimmen
        $outputFolder = "";
        foreach ($felder as $feld) {
            if ($feld['fk_formular_id'] === $selected_formular['id'] && !empty($feld['folder'])) {
                $outputFolder = $feld['folder'];
                break;
            }
        }
        
        if (!empty($outputFolder)) {
            $filePath = $_SERVER['DOCUMENT_ROOT'] . "/" . trim($outputFolder, "/") . "/formular_" . preg_replace('/[^a-zA-Z0-9_]/', '_', $selected_formular['label']) . ".html";
            file_put_contents($filePath, $generatedForm);
            echo "<p>Formular wurde gespeichert unter: <code>$filePath</code></p>";
        }
    endif; ?>
</div>
